...
이 기록을 바탕으로, 실제 채용 담당자가 읽는 포트폴리오처럼 **STAR 기법(상황-과제-행동-결과)**과 **XYZ 형식("~를 ~함으로써 ~를 달성/개선")**으로 정리해주세요. 수치는 기록에 있을 때만 쓰고, 없으면 정성적으로 표현합니다. 막연한 나열이 아니라 "무엇을 어떻게 해결했는지"가 드러나야 합니다.
...
- 과제(Task):
- 행동(Action):
- 결과(Result): (기록에 수치가 있을 때만 수치로, 없으면 구체적인 정성적 결과로)

## 핵심 성과 요약
(불릿 4~6개, "~함으로써 ~를 달성" 형태 지향, 수치는 기록에 있을 때만, 각 불릿도 배경과 과정을 살려 구체적으로)
규칙:
