...
| 영역 | 기능 |
|---|---|
| 기록 | 캘린더 기반 3줄 기록, 기록 수정, 로그당 여러 분류 선택, 프로젝트별 소개 입력 |
| 습관 | 연속 기록 카드(스트릭·이번 주 기록 현황·최장 연속), 오늘 기록 전 끊김 경고 |
| GitHub | 저장소 커밋 불러오기 → 원하는 커밋만 골라 기록에 추가 (내 커밋만 필터, 중복 방지), 커밋 접두사(feat/fix/docs)로 분류 자동 지정, 저장소 사용 기술을 프로젝트 소개에 반영 |
...
| 보관함 | 포트폴리오·이력서·자소서 종류별 탭, 삭제 |
| 계정 | 이메일 가입/로그인, 비밀번호 찾기, 로그인 없이 체험(익명 계정) |

### 신뢰성 설계

- 기록에 없는 사실·수치를 쓰지 않도록 모든 프롬프트에 공통 근거 규칙을 적용하고, 결과에 기록에 없는 %/배 수치가 있으면 재생성 후 제거
- 결과 아래에 "근거: 내 기록 N개 (기간)"을 표시하고, 항목마다 근거 날짜를 인용
